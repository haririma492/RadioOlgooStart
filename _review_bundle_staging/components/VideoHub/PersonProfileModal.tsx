...
export default function PersonProfileModal({
  ...
  const handleNext = () => {
    if (!scrollContainerRef.current) return;
    const container = scrollContainerRef.current;
    const firstCard = container.querySelector("[data-video-card]") as HTMLElement | null;
    if (!firstCard) return;
  ...
  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container || videos.length === 0 || !isOpen) return;

    const checkWithRAF = () => requestAnimationFrame(checkScrollPosition);
    checkWithRAF();

    const timeoutId = window.setTimeout(checkWithRAF, 200);
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="relative flex w-full max-w-6xl flex-col rounded-xl border border-white/10 bg-[#1a1a1a] mx-4 scrollbar-hide"
          style={{
            pointerEvents: "auto",
            maxHeight: "55vh",
          }}
                    ...
                    >
                      <div className="relative w-full aspect-video rounded-lg overflow-hidden border border-white/20 bg-black">
                        {video.thumbnail ? (
                          <img
                            src={video.thumbnail}
                            alt={video.title || video.personName || video.person || "Video thumbnail"}
                        ...
                        <div className="absolute inset-0 flex items-center justify-start pl-3 bg-black/30 hover:bg-black/20 transition-colors pointer-events-none">
                          ...
                          >
                            ...
                            <path d="M20 16L32 24L20 32V16Z" fill="#212B36" />
                          </svg>
                        </div>
                      </div>

                      {video.title && (
                        <div className="text-white text-sm mt-2 truncate">{video.title}</div>
                      )}
