...
export default function ProfileCardWithDropdown({
  ...
  return (
    <div
      ...
      className={`relative ${
        isKing
          ? "max-w-[360px] lg:max-w-[440px] xl:max-w-[510px] w-full h-full max-h-[calc(100%-50px)] min-h-0 flex flex-col"
          : ""
      }`}
    >
      <div
      ...
      >
        <div
          className={`relative overflow-hidden border border-white/20 bg-black ${
            isKing
              ? "rounded-2xl flex-1 min-h-0 min-w-0 w-full aspect-square max-h-full max-w-full"
        ...
        <div
          className={`text-white text-sm text-center truncate ${
            isKing ? "mt-2 flex-shrink-0" : "mt-1"
          }`}
        >
          {profile.person}
        </div>
